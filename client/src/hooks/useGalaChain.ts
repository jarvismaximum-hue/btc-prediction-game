...
export function useGalaChain() {
  ...
  const fetchBalance = useCallback(async () => {
    ...
    try {
      ...
      // result.Data is an array of balance entries
  ...
  const depositToGame = useCallback(async (amount: number, platformWallet: string): Promise<{ success: boolean; error?: string }> => {
    ...
    try {
      await tokenApiRef.current.TransferToken({
        ...
        uniqueKey: `profitplay-deposit-${Date.now()}-${Math.random().toString(36).slice(2)}`,
      });
